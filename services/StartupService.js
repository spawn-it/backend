...
class StartupService {
    ...
    static async initializeAllPlanLoops(bucketName) {
        console.log('[StartupService] Starting plan loops for all existing services...');
        ...
        try {
            ...
            for (const clientId of clients) {
                try {
                    ...
                    console.log(`[StartupService] Client ${clientId}: ${serviceIds.length} services found`);

                    for (const serviceId of serviceIds) {
                        try {
                            // Skip services that have errors in their info
                            if (services[serviceId]?.error) {
                                console.warn(`[StartupService] Skipping service ${clientId}/${serviceId}: has errors`);
                                continue;
                            }

                            // Prepare working directory and start plan loop
                            const dataDir = await workingDirectoryService.prepare(clientId, serviceId, bucketName);
                            planLoopManager.start(clientId, serviceId, dataDir);

                            summary.servicesStarted++;
                            console.log(`[StartupService] Started plan loop for ${clientId}/${serviceId}`);
    ...
    static stopAllPlanLoops() {
        ...
        console.log(`[StartupService] Stopping ${activeLoops.length} active plan loops`);

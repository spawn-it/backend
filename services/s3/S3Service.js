...
class S3Service {
    ...
    async listClients(bucket) {
        try {
            const data = await this.client.send(new ListObjectsV2Command({
                Bucket: bucket,
            ...
            console.error(`[S3Service] Error listing clients in bucket ${bucket}:`, err);
            throw err;
        }
    }
        ...
        try {
            ...
            const serviceNames = (data.CommonPrefixes || [])
                ...
                .filter(serviceName => {
                    ...
                    if (!isValid) {
                        console.warn(`[S3Service] Filtering out invalid service name: ${serviceName}`);
                    }
                    return isValid;
                });
            ...
            for (const service of serviceNames) {
                try {
                    ...
                    if (!this._isValidObjectName(infoKey)) {
                        console.warn(`[S3Service] Skipping service with invalid key: ${infoKey}`);
                        continue;
                    }
                    ...
                    } else if (err.name === 'XMinioInvalidObjectName' || err.code === 'InvalidObjectName') {
                        console.warn(`[S3Service] Skipping service with invalid object name: ${service}`);
                        continue;
                    } else {
                        console.error(`[S3Service] Error getting info for service ${service}:`, err);
            ...
            if (err.name === 'XMinioInvalidObjectName') {
                console.error(`[S3Service] MinIO rejected the request due to invalid object name for client ${clientId}`);
                console.error(`[S3Service] This might be due to special characters in the client ID`);
                return {}; // Return empty services instead of throwing
            }

            console.error(`[S3Service] Error listing services for client ${clientId}:`, err);
            throw err;
        }
    }
    ...
    async createFile(bucket, key, content, contentType = "application/json") {
        ...
        if (!this._isValidObjectName(key)) {
            ...
            console.warn(`[S3Service] Object name sanitized: ${key} -> ${sanitizedKey}`);
            key = sanitizedKey;
        }
    ...
    async getFile(bucket, key) {
        try {
            ...
            if (err.name === 'XMinioInvalidObjectName' || err.code === 'InvalidObjectName') {
                const error = new Error(`Invalid object name: ${key}`);
                error.code = 'InvalidObjectName';
                throw error;
            }
    ...
    async downloadFiles(bucket, prefix, destinationFolder) {
        ...
        for (const file of list.Contents || []) {
            ...
            if (!this._isValidObjectName(fileKey)) {
                console.warn(`[S3Service] Skipping file with invalid object name: ${fileKey}`);
                continue;
            }
        ...
        try {
            ...
            for (const object of listResponse.Contents) {
                ...
                if (!this._isValidObjectName(object.Key)) {
                    console.warn(`[S3Service] Skipping deletion of invalid object: ${object.Key}`);
                    continue;
                }
                ...
                try {
                    ...
                    console.warn(`[S3Service] Failed to delete ${object.Key}:`, deleteErr.message);
                    // Continue with other files
                }
            }

        } catch (err) {
            console.error(`[S3Service] Error deleting files with prefix ${prefix}:`, err);
            throw err;
        }
    }
}
